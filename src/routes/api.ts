...
아래 텍스트에서 모든 서비스와 가격 정보를 **누락 없이 정확하게** 추출하세요.
...
## 출력 형식 (JSON만 출력, 반드시 이 형식 준수)
{
  "menuText": "서비스명 - 가격 형식으로 줄바꿈 구분. 할인이 있으면: 서비스명 - 정가 → 할인가 (할인율)",
  "operatingHours": "영업시간 또는 null",
  "systemPrompt": "시스템 프롬프트 전체 내용"
}
...
## 🎉 현재 진행 중인 이벤트 (첫방문/오픈 할인)
[텍스트에서 할인 이벤트를 모두 추출하여 아래 형식으로 작성]
- 서비스명: 정가 → 할인가 (할인율%) - 간단한 설명
예시:
...
- 영업시간
- 전화번호: 031-235-5726
- 예약금: 20,000원 (방문 시 차액 결제)
- VAT 별도

## 📌 응대 지침
- 가격 문의 시 **정확한 가격**과 **현재 이벤트**를 반드시 함께 안내
- "가격 변동"이라고 하지 말고 실제 가격을 안내
- 예약으로 마무리

## 중요 규칙:
1. **"가격 변동"이라고 쓰지 마세요** - 텍스트에 있는 실제 가격을 그대로 사용
2. 할인 이벤트는 "정가 → 할인가 (할인율)" 형식 필수
3. 모든 서비스를 빠짐없이 포함
4. JSON만 출력, 마크다운 코드블록 금지`;
